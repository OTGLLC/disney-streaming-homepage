...
Tile::Tile(std::string name,glm::vec2 pos, glm::vec2 size, Texture image,int rowPosition,glm::vec2 velocity)
	: TileName(name),Position(pos), Size(size), Velocity(velocity), Image(image),ColumnPosition(rowPosition),SelectedSize(size*1.1f),DisplaySize(size) { }

void Tile::Draw(TextureRenderer& renderer)
{
    
	renderer.DrawTexture(this->Image, this->Position, this->DisplaySize);
}